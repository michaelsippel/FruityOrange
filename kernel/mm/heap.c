...
void init_heap(void) {
  first_nd = mk_nd(pmm_alloc(), PAGE_SIZE, NULL, NULL);
}
...
void *malloc(size_t bytes) {
  ...
  while (node != NULL) {
	...
	first_nd->prev_nd = mk_nd((uintptr_t)node+node->bytes-rest, rest, first_nd, NULL);
  ...
  first_nd->prev_nd = mk_nd(pmm_alloc(), PAGE_SIZE, first_nd, NULL);
  first_nd = first_nd->prev_nd;
